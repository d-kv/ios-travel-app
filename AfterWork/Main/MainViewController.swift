...
class MainViewController: UIViewController {
  ...
  override func viewDidLoad() {
    ...
    mainViewPresenter.delegate = self
  ...
  @objc
  func checkAllButtonTap() {
    mainViewPresenter.goToMap()
  }

  @objc
  func recommendButtonTap() {
  ...
  func creation() {
    ...
    let gesturePersonal = UITapGestureRecognizer(
      target: self,
      action: #selector(recommendButtonTap)
    )
    gesturePersonal.numberOfTapsRequired = 1
    let gestureBig = UITapGestureRecognizer(target: self, action: #selector(recommendButtonTap))
    gestureBig.numberOfTapsRequired = 1
    let gestureLeft = UITapGestureRecognizer(
      target: self,
      action: #selector(recommendButtonTap)
    )
    gestureLeft.numberOfTapsRequired = 1
    let gestureCenter = UITapGestureRecognizer(
      target: self,
      action: #selector(recommendButtonTap)
    )
    gestureCenter.numberOfTapsRequired = 1
    let gestureRight = UITapGestureRecognizer(
      target: self,
      action: #selector(recommendButtonTap)
    )
  ...
  func setUpConstraints() {
    ...
    let leftRecommendConstraints = [
      ...
      leftRecommend.rightAnchor.constraint(
        equalTo: centerRecommend.leftAnchor,
        constant: -20
      ),
    ...
    let rightRecommendConstraints = [
      ...
      rightRecommend.leftAnchor.constraint(
        equalTo: centerRecommend.rightAnchor,
        constant: 20
      ),
    ...
    let personalRecommendConstraints = [
      personalRecommend.topAnchor.constraint(
        equalTo: centerRecommend.bottomAnchor,
        constant: 20
      ),
    ...
    let checkAllButtonConstraints = [
      checkAllButton.topAnchor.constraint(
        equalTo: personalRecommend.bottomAnchor,
        constant: 20
      ),
    ...
    let constraintsArray = [
      ...
      bigRecommendConstraints,
      centerRecommendConstraints,
      leftRecommendConstraints,
      rightRecommendConstraints,
      personalRecommendConstraints,
      checkAllButtonConstraints,
    ].flatMap { $0 }
  ...
  func addto_bigRecommend() {
    let bigText = DI.shared.getInterfaceExt()
      .frameTextView(text: "Кафе", font: .boldSystemFont(ofSize: 24), lineHeightMultiple: 0.6)
    let smallText = DI.shared.getInterfaceExt().frameTextView(
      text: "Для быстрого перекуса",
    ...
    let constraintsArray = [bigTextConstraints, smallTextConstraints, imageViewConstraints]
      .flatMap { $0 }
    NSLayoutConstraint.activate(constraintsArray)
  }
  ...
  func addto_centerRecommend() {
    let bigText = DI.shared.getInterfaceExt()
      .frameTextView(text: "Отель", font: .boldSystemFont(ofSize: 16), lineHeightMultiple: 0)
  ...
  func addto_leftRecommend() {
    let bigText = DI.shared.getInterfaceExt()
      .frameTextView(text: "Ресторан", font: .boldSystemFont(ofSize: 16), lineHeightMultiple: 0)
  ...
  func addto_rightRecommend() {
    let bigText = DI.shared.getInterfaceExt()
      .frameTextView(text: "Культура", font: .boldSystemFont(ofSize: 16), lineHeightMultiple: 0)
  ...
  func addto_personalRecommend() {
    let bigText = DI.shared.getInterfaceExt().frameTextView(
      text: "Персональные рекомендации",
      font: .boldSystemFont(ofSize: 24),
      lineHeightMultiple: 0.8
    )
    let smallText = DI.shared.getInterfaceExt().frameTextView(
      text: "Может, это именно то что нужно",
      font: .systemFont(ofSize: 14),
      lineHeightMultiple: 0.8
    )
