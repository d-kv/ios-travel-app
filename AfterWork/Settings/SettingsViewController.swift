...
class SettingsViewController: UIViewController {
  ...
  let signOutButton = DI.shared.getInterfaceExt()
    .lightGreyButton(title: "Выйти из аккаунта", color: UIColor(named: "LightGrayColor")!)

  let resetRecommendButton = DI.shared.getInterfaceExt()
    .lightGreyButton(title: "Сбросить рекомендации", color: UIColor(named: "LightGrayColor")!)
  let adminButton = DI.shared.getInterfaceExt()
    .lightGreyButton(title: "Админ-панель", color: UIColor(named: "LightGrayColor")!)
  ...
  func setUpConstraints() {
    ...
    let resetRecommendButtonConstraints = [
      resetRecommendButton.bottomAnchor.constraint(
        equalTo: signOutButton.topAnchor,
        constant: -20
      ),
    ...
    let adminButtonConstraints = [
      adminButton.bottomAnchor.constraint(
        equalTo: resetRecommendButton.topAnchor,
        constant: -20
      ),
    ...
    let constraintsArray = [
      ...
      userNameConstraints,
      achievementsViewConstraints,
      signOutButtonConstraints,
      resetRecommendButtonConstraints,
      adminButtonConstraints,
    ].flatMap { $0 }
  ...
  func achievementTap() {
    let alert = UIAlertController(
      title: "Хороший отдых",
      message: "Из-за трясущихся рук не в состоянии тыкнуть на нужную кнопку",
      preferredStyle: .alert
    )
