...
class CardsViewController: UIViewController, SwipeCardStackDataSource, SwipeCardStackDelegate, CLLocationManagerDelegate {
    ...
    var cards: [Places] = CardsViewPresenter.getCards() ?? [
        Places(id: 0, yaid: 0, category: String(localized: "instruction_title"),
    ...
    @objc private func cardTap() {
        ...
        let coordinate = CLLocationCoordinate2D(latitude: Double(nextCard.latitude) ?? 0,
                longitude: Double(nextCard.longitude) ?? 0)
        ...
        let text = NSAttributedString(
            string: "Подборка пока что закончилась,\nзагляните позже",
                  attributes: [
                    ...
                    NSAttributedString.Key.font: UIFont(name: "Helvetica Neue", size: 28) ?? .systemFont(ofSize: 28),
                    NSAttributedString.Key.foregroundColor: UIColor(named: "LightGrayColor") ?? .black
                  ]
        )
