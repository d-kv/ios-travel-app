...
class CardsViewController: UIViewController, SwipeCardStackDataSource, SwipeCardStackDelegate,
  ...
  let cards = [
    [
      "Инструкция",
      "Что это такое?",
      "Смахнешь влево - получишь карточки, смахнешь вправо - отдохни на браво. А карточки с подсветкой - наши избранные, имей ввиду",
      "текст",
      4,
      false,
      56.2965039,
    ...
    [
      "Пиццерия",
      "Papa Jhones",
      "Третья крупнейшая сеть пиццерий в мире после Pizza Hut и Domino's Pizza, на конец 2018 года сеть Papa John's включала 5303 пиццерии, из них 645 были собственными, остальные работали по франчайзингу",
      "Ежедневно с 12:00 - 00:00",
      4,
      false,
      56.2965039,
      44.9360589,
    ...
    if direction == .right, index != 0 {
      ...
      let artwork = Artwork(
        ...
        coordinate: CLLocationCoordinate2D(
          latitude: cards[index + 1][6] as! CLLocationDegrees,
          longitude: cards[index + 1][7] as! CLLocationDegrees
        ), // cards[0][6]
  ...
  func cardStack(
    _ cardStack: Shuffle_iOS.SwipeCardStack,
    cardForIndexAt index: Int
  )
    -> Shuffle_iOS.SwipeCard {
    ...
    let artwork = Artwork(
      ...
      discipline: "Sculpture",
      coordinate: CLLocationCoordinate2D(
        latitude: cards[index][6] as! CLLocationDegrees,
        longitude: cards[index][7] as! CLLocationDegrees
      ), // cards[0][6]
      image: UIImage(named: "markerTop")
    )
  ...
  func card(fromData data: [Any]) -> SwipeCard {
    ...
    let content = DI.container.resolve(InterfaceExt.self)!.basicCard(
      type: data[0] as! String,
      title: data[1] as! String,
      description: data[2] as! String,
      workHours: data[3] as! String,
      bill: data[4] as! Int,
      isRecommended: data[5] as! Bool
    )
  ...
  override func viewDidLoad() {
    ...
    DI.container.resolve(CardsViewPresenter.self)?
  ...
  func create() {
    ...
    let text = NSAttributedString(
      string: "Подборка пока что закончилась,\nзагляните позже",
      attributes: [
        NSAttributedString.Key.paragraphStyle: style,
        NSAttributedString.Key.font: UIFont(name: "Helvetica Neue", size: 28)!,
        NSAttributedString.Key.foregroundColor: UIColor(named: "LightGrayColor")!,
      ]
    )
